    ...
    ) : PhotoStandardSize, IPhotoService
{
    ...
    public async Task<string[]?> AddPhotoToDiskAsync(IFormFile formFile, string productId)
    {
        ...
        if (formFile.Length > 0) // 301kb => 301_000 byte
        {
            ...
            string filePath_165_sq = await _photoModifyService.ResizeByPixel_Square(formFile, productId, 165); // navbar & thumbnail
            string filePath_256_sq = await _photoModifyService.ResizeByPixel_Square(formFile, productId, 256); // card
            string filePath_enlarged = await _photoModifyService.ResizeImageByScale(formFile, productId, (int)DimensionsEnum._4_3_800x600); // enlarged photo
